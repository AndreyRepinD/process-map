...
STEP_DESCRIPTIONS: dict[str, dict[str, str]] = {
    ...
    "meio": {
        ...
        "raschet-volatilnosti-sprosa": "Коэффициент вариации спроса по связке продукт-локация: насколько разбросано потребление вокруг своего среднего.\n\nБаза неопределённости — ПАРАМЕТР НАСТРОЙКИ, а не жёсткое правило: модуль умеет считать разброс сам по собственной истории, а может принять готовую ошибку прогноза (FA, BIAS, σ) по горизонту из DP. Выбор делается по сегментам: страховой запас защищает от ошибки прогноза за время выполнения, и там, где прогноз смещён, собственный разброс отгрузок занижает потребность в защите.",
        ...
        "otchet-po-oshibke-prognoza": "Отчёт по расхождению прогноза и факта: FA, BIAS и σ по горизонту.\n\nВ платформе такой алгоритм ЕСТЬ — то есть ошибка прогноза как база неопределённости модулю доступна, и её отсутствие на прежней версии карты было пробелом карты, а не продукта. Отсюда же берётся вход «База неопределённости» на этапе 2: контур замыкается на следующем цикле.",
