...
MAP_UPDATED_AT_DP = "2026-09-17"
MAP_DATA_FINGERPRINT_DP = "798140b159de7e033d05de92d8cc6a9045a2f6545634bc79e94d69630bfc77c2"

MAP_ID_MEIO = "meio"
MAP_TITLE_MEIO = "Процесс мультиэшелонной оптимизации запасов"
MAP_MODULE_LABEL_MEIO = "Модуль MEIO"
MAP_UPDATED_AT_MEIO = "2026-09-17"
MAP_DATA_FINGERPRINT_MEIO = "4c2b05f583c1bf9a03fe029cc3f5005341f732a4722c563c8b02f7a53357de87"
...
STEP_DESCRIPTIONS: dict[str, dict[str, str]] = {
    "dp": {
        "proverka-kachestva-dannyh": "Гейт качества до прогона: полнота истории Sell-In, Sell-Out, Off-Take и заказов, пропуски, новые и изменённые DFU и маппинг SKU, единицы измерения, дубли и отрицательные значения.\n\nЦелевое правило (реализация в In.Plan не подтверждена): критическая ошибка набора данных останавливает расчёт; локальная ошибка изолирует затронутые ряды в карантин, остальной портфель считается. После прогона — отчёт охвата: сколько рядов ожидалось, обработано и исключено.\n\nПравила: регламент DP, §3.\n\nBPMN: DP-010 — Подготовка данных",
        "podgotovka-sopostavimoy-istorii": "Очистка и приведение истории продаж к сопоставимому виду: OOS, выбросы, промо, ML-очистка. Перенос истории при смене логистической схемы, замена SKU через DFU, исключения планеру.\n\nОчистка автоматическая и экспертная: модель предлагает правку, спорные случаи уходят планеру исключениями. Календари дефицитов, прошлых промо и событий отличают провал продаж из-за OOS от падения спроса.\n\nПравила: регламент DP, §4.\n\nBPMN: DP-020 — Очистка данных",
        "segmentaciya-klasterizaciya-i-nastroyka-metodov": "Сегментация и кластеризация номенклатуры по признакам рядов: ABC/XYZ, непрерывность, тренд, ML-кластеризация, сезонность. Ряды тестируются, и к сегменту привязываются методы и параметры прогнозирования.\n\nСюда возвращается ретроспектива точности с этапа 5: сегменты и методы пересматриваются по FA, BIAS и FVA прошлых циклов.\n\nПравила: регламент DP, §5.\n\nBPMN: DP-040 — Управление сегментами (семействами)",
        "assortiment-metody-i-chempion": "Расчёт модельного прогноза по ассортименту: статистические методы, ансамбли и многофакторная ML-модель спроса. Модели запускаются по настройкам сегмента и по расписанию; чемпион для каждой серии выбирается по backtest.\n\nАссортиментная матрица берётся на период прогноза — с запланированными запусками и датами вывода, а не только на сегодня. Базовый прогноз уходит в NRM, чтобы промо-план считался от той же базы.\n\nПравила: регламент DP, §5, §6.\n\nBPMN: DP-060 — Прогнозирование базовой линии",
        "novinki-pohozhie-tovary": "Прогноз новинок по похожим товарам: референс (аналог), коэффициент пересчёта и даты действия связи.\n\nПравило подобия и коэффициенты каннибализации приходят из ML-модели ассортиментной матрицы. Каннибализация снижает прогноз товаров, которые новинка замещает.\n\nПравила: регламент DP, §6.\n\nBPMN: DP-070-020 — Планирование новинки",
        "promo-light-prognoz-promo-obemov-opcionalno": "Прогноз промо-объёмов: календарь акций и механик; промо-объёмы по промо-эффектам из истории или промо-план из TPM/NRM. В NRM промо-объёмы считает своя ML-модель промо с учётом ценовой эластичности.\n\n«Опционально» означает, что шаг может быть пропущен по содержанию: если промо нет, прогноз проходит шаг без изменений.\n\nОткрытый вопрос (проверяется на примере акции в In.Plan): промо-объёмы — это прирост к базе или полный объём акции, и учитывает ли модель-чемпион будущие акции сама. От ответа зависит, можно ли складывать промо-объёмы с прогнозом без двойного учёта.\n\nПравила: регламент DP, §2, §7.\n\nBPMN: DP-070-050 — Анализ промо-плана",
        "pryamye-kanaly-pryamoy-prognoz-sell-in": "Прямой прогноз Sell-In для клиентов, которых модель канала относит к прямым продажам.\n\nМаршрут выбирается для каждого клиента: прямой прогноз или баланс дистрибьютора. Обе ветви дают общий результат — прогноз Sell-In по каналам.\n\nПравила: регламент DP, §8.",
        "sell-out-balans-sell-in": "Пересчёт прогноза Sell-Out в Sell-In через материальный баланс клиента: остатки, транзит, открытые заказы, целевой запас; прогнозный остаток и дни покрытия.\n\nНа вход — прогноз Sell-Out с учтёнными эффектами (результат этапа 2 для канала через дистрибьютора) и данные клиента: Sell-Out, остатки, транзит и открытые заказы приходят от сетей и дистрибьюторов (EDI, порталы), а не из собственной ERP. Рост Sell-In может быть заполнением склада дистрибьютора, а не ростом потребления.\n\nПравила: регламент DP, §8.\n\nBPMN: DP-070-070 — Demand Propagation. Автоматизированный расчет Sell-In дистрибутора",
        "building-blocks-i-korrektirovki": "Обогащение прогноза блоками Building Blocks: объёмные, процентные, замещающие — в том числе листинг, делистинг и запуск новинок. Распределение вниз и пересчёт вверх, версии и аудит правок.\n\nЭффект, уже учтённый на этапе 2 — промо или новинка, — блоком повторно не вводится: иначе он попадёт в план дважды.\n\nПравила: регламент DP, §2, §9.\n\nBPMN: DP-070 — Обогащение прогноза",
        "demand-review-meeting": "Согласование прогноза на Demand Review Meeting по дашбордам и аналитическим отчётам: консенсус в объёме и, при наличии цен, в деньгах; сравнение версий и сценариев; решения по расхождениям и исключениям.\n\nSales и KAM приносят коммерческие допущения: клиенты, листинги, промо, контракты, риски и возможности. Коммерческие финансы дают цены, бюджет и цели, оценку плана в деньгах и объяснение разрыва с бюджетом. Бюджет не подменяет прогноз спроса.\n\nИтог встречи — явно утверждённая версия плана спроса: фиксируются утверждающий, дата, решения по расхождениям и исходные предложения.\n\nПравила: регламент DP, §9.\n\nBPMN: DP-080-010 — Анализ скорректированного прогноза по итогам DRM",
        "publikaciya-plana-sprosa": "Дезагрегация DFU → SKU и публикация плана спроса с лагом, передача неограниченного плана в SNP и MEIO с фиксацией версии. Каждая опубликованная версия ложится в архив по лагам.\n\nПоглощение прогноза заказами выполняется один раз и не в DP: на стенде In.Plan это процесс «SNP Поглощение прогноза» (проверено 17.09.2026), в модели процесса — SP-010-020, на карте SNP — этап 1. DP публикует план без вычета заказов.\n\nКонтракт с MEIO: прогноз спроса и показатели ошибки прогноза по горизонту и уровню детализации. Волатильность продаж — другой показатель, MEIO считает её по истории отгрузок сам.\n\nПравила: регламент DP, §10.\n\nBPMN: DP-080-030 — Дезагрегация DFU->SKU и публикация прогноза",
        "sverka-s-ogranichennym-planom-snp-i-reshenie-po-razryvu": "Сопоставление опубликованного неограниченного плана спроса с ограниченным планом SNP и решение по разрыву: приоритизация клиентов и SKU, перераспределение, перенос промо.\n\nРешение совместное: Supply готовит варианты, Sales и Marketing подтверждают коммерческие изменения, DP обновляет согласованный спрос. Неразрешённые компромиссы выносятся на S&OP. Перенос акции возвращается в календарь промо и согласование; само ограничение поставки исходный неограниченный прогноз не уменьшает.\n\nПравила: регламент DP, §11.",
        "kontrol-tochnosti-i-fva": "Оценка точности по версии прогноза и лагу: FA, MAPE, BIAS, WAPE. MAPE не считается на нулевом факте, WAPE не определён при нулевой сумме факта. Знак BIAS задаётся формулой In.Plan и на стенде ещё не сверен.\n\nFVA показывает вклад каждого слоя: наивный, модельный (статистика и ML), с учтёнными эффектами, после Building Blocks, консенсус. Требование: сопоставимые снимки этих слоёв на дату решения. Архив публикаций по лагам в In.Plan есть; хранение промежуточных слоёв не проверено.\n\nРезультат возвращается на этап 2, а ошибка прогноза по горизонту уходит в MEIO.\n\nПравила: регламент DP, §12.\n\nBPMN: DP-030 — Расчет метрик точности прогноза",
        "analiz-preduprezhdeniy": "Разбор предупреждений цикла: точность ниже порога по FA или BIAS, разрыв с ограниченным планом SNP, прогноз без истории по новому DFU, ручная корректировка без обоснования.\n\nПриоритет — по влиянию: деньги, объём, уровень сервиса и критичность позиции. Отсутствие цены не скрывает исключение: его приоритет оценивается в объёме или по сервису. У каждого предупреждения назначается владелец.\n\nПравила: регламент DP, §13.",
    },
    "meio": {
...
STAGE_INPUT_CARDS: dict[str, dict[str, tuple[str, ...]]] = {
    "dp": {
        ...
        "stage-2-raschet-prognoza": (
            ...
            "Данные для feature engineering",
        ...
        "stage-3-sell-in-po-modeli-kanala": (
            ...
            "Целевой запас клиента в днях покрытия",
...
STAGE_WIRING: dict[str, dict[str, dict[str, object]]] = {
    "dp": {
        "stage-1-podgotovka-istorii": {
            "inputsTo": {
                "Логистическая схема и её изменения": "podgotovka-sopostavimoy-istorii",
                "Календарь OOS и дефицитов": "podgotovka-sopostavimoy-istorii",
                "Календарь промо прошлых периодов": "podgotovka-sopostavimoy-istorii",
                "Календарь событий": "podgotovka-sopostavimoy-istorii",
            },
            "outputsFrom": {
        ...
        "stage-2-raschet-prognoza": {
            "inputsTo": {
                "Ассортиментная матрица": "assortiment-metody-i-chempion",
                "Данные для feature engineering": "assortiment-metody-i-chempion",
                "Коэффициенты подобия и каннибализации": "novinki-pohozhie-tovary",
                "Календарь акций и механик": "promo-light-prognoz-promo-obemov-opcionalno",
            },
            "outputsFrom": {
        ...
        "stage-3-sell-in-po-modeli-kanala": {
            "inputsTo": {
                "Модель канала по клиентам": ["pryamye-kanaly-pryamoy-prognoz-sell-in", "sell-out-balans-sell-in"],
                "Целевой запас клиента в днях покрытия": "sell-out-balans-sell-in",
                "Прогноз Sell-Out с учтёнными эффектами": "sell-out-balans-sell-in",
            },
            "outputsFrom": {
                "Прогноз Sell-In по каналам": ["pryamye-kanaly-pryamoy-prognoz-sell-in", "sell-out-balans-sell-in"],
                "Прогнозный остаток и дни покрытия у клиента": "sell-out-balans-sell-in",
            },
        },
        "stage-5-publikaciya-i-kontrol-tochnosti": {
            "inputsTo": {
                "Факт продаж за закрытый период": "kontrol-tochnosti-i-fva",
                "Пороги точности и правила алертов": "kontrol-tochnosti-i-fva",
            },
            "outputsFrom": {
...
def wiring_targets(value: object) -> list[str]:
    """
    Шаги, названные в STAGE_WIRING для одной подписи: строка или список строк.

    ЗАЧЕМ СПИСОК (решение владельца 17.09.2026, п.7 разбора аудита карты DP).
    На этапе «Sell-In по модели канала» обе ветви — прямые каналы и баланс
    дистрибьютора — дают один результат «Прогноз Sell-In по каналам», а модель
    канала нужна обеим. При одном потребителе и одном производителе на карточку
    прямая ветвь выглядела тупиком: её результат никуда не шёл. Одиночная строка
    по-прежнему значит ровно один шаг, поэтому прежние таблицы не меняются.
    """
    if value is None:
        return []
    ...
    if isinstance(value, (list, tuple)) and all(isinstance(item, str) for item in value):
        return list(value)
    raise SystemExit(f"STAGE_WIRING: ожидалась строка или список строк, получено {value!r}")
...
def input_cards(
    ...
    for index, label in enumerate(labels):
        ...
        nodes.append(node)
        for consumer in wiring_targets(consumers.get(label, default_consumer)):
            edges.append(
                {
...
def result_nodes(
    ...
    for index, label in enumerate(key_outputs):
        ...
        for producer in wiring_targets(producers.get(label, default_producer)):
            edges.append(
                {
...
def build_single_slide_map(
    ...
    for meta in stage_meta:
        ...
        consumers: dict[str, object] = raw_consumer if isinstance(raw_consumer, dict) else {}
        default_consumer = (
...
def run_self_test() -> int:
    ...
    # STAGE_WIRING: строка — один шаг, список — несколько, прочее — остановка.
    check(wiring_targets("a") == ["a"], "строка в STAGE_WIRING не дала одного шага")
    check(wiring_targets(["a", "b"]) == ["a", "b"], "список в STAGE_WIRING потерял шаги")
    check(wiring_targets(None) == [], "отсутствие привязки дало ребро")
    try:
        wiring_targets(["a", 1])
    except SystemExit:
        rejected = True
        ...
        rejected = False
    check(rejected, "нестроковый шаг в STAGE_WIRING принят молча")
