...
MAP_DATA_FINGERPRINT_MEIO = "c2e1b30bc04a091274a2dacfd351cad88e8bfc56efe83beb5430ca74c4cb9b1e"
...
STEP_DESCRIPTIONS: dict[str, dict[str, str]] = {
    ...
    "meio": {
        ...
        "raschet-volatilnosti-sprosa": "Коэффициент вариации спроса по связке продукт-локация: насколько разбросано потребление вокруг своего среднего.\n\nБаза неопределённости выбирается ПО ПРИОРИТЕТУ, а не двумя источниками разом. Страховой запас защищает от ошибки ПРОГНОЗА за время выполнения, поэтому готовая оценка ошибки (FA, BIAS, σ по горизонту) из DP — источник первого выбора: она уже учитывает смещение прогноза. Если по позиции или горизонту DP её не публикует, модуль считает разброс сам — по собственной истории прогноза и факта, тем же расчётом, что стоит в отчёте по ошибке прогноза на этапе 4.\n\nРазброс исторических ОТГРУЗОК — крайний случай, а не равноправная опция: там, где прогноз смещён, он занижает потребность в защите. Выбор делается по сегментам и остаётся настройкой владельца процесса.",
        ...
        "korrektirovka-cv": "Сведение трёх посчитанных коэффициентов вариации к тому набору, с которым пойдёт оптимизация, и экспертная правка там, где история не отражает будущее.\n\nШаг ручной: алгоритма в платформе у него нет. Он нужен, потому что посчитанный по истории CV врёт в двух известных случаях — новинка без истории и позиция, у которой режим поставки только что изменился. Без явного шага правка всё равно происходит, но невидимо и без следа.",
        "abc-klassifikaciya": "Разнесение номенклатуры по классам A, B и C — по вкладу в оборот и объём.\n\nВынесено отдельным шагом, потому что это отдельный алгоритм платформы и отдельное решение о порогах: класс определяет, какой уровень сервиса позиция вообще заслуживает, и меняется он не тогда же, когда остальные признаки сегментации.",
    ...
    "meio": {
        ...
        "stage-2-podgotovka-k-raschetu": {
            "outputsFrom": {
                ...
                "Скорректированный CV": "korrektirovka-cv",
...
STAGE_GROUPS: dict[str, dict[str, tuple[tuple[str, tuple[str, ...]], ...]]] = {
    ...
    "meio": {
        ...
        "stage-2-podgotovka-k-raschetu": (
            ...
            ("Расчёт вариабельности", ("raschet-volatilnosti-sprosa", "raschet-cv-dlitelnosti-postavok", "raschet-cv-dlitelnosti-proizvodstva", "korrektirovka-cv", )),
            ("Сегментация", ("abc-klassifikaciya", "segmentaciya", )),
        ),
        "stage-3-raschet-i-analiz": (
