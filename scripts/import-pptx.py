...
STEP_DESCRIPTIONS: dict[str, dict[str, str]] = {
    ...
    "meio": {
        ...
        "nastroyka-parametrov-dlya-rascheta-urovney-zapasov": (
            ...
            "срок годности и политика FEFO.\n\n"
            "База неопределённости — ПАРАМЕТР НАСТРОЙКИ, а не жёсткое правило: модуль "
            "умеет считать разброс сам по собственной истории прогноза и факта, а может "
            "принять готовую ошибку прогноза (FA, BIAS, σ) по горизонту из DP. Выбор "
            "делается по сегментам: страховой запас защищает от ошибки прогноза за время "
            "выполнения, и там, где прогноз смещён, собственный разброс отгрузок занижает "
        ...
        "raschet-rekomendaciy-po-urovnyam-zapasov": (
            ...
            "уровень сервиса.\n\n"
            "Срок годности — жёсткое ограничение, а не пожелание: целевое покрытие, "
            "превышающее остаточный срок охлаждённой продукции, превращается не в "
            "излишек, а в списание. Поэтому покрытие ограничено остаточным сроком с "
            "учётом требований сетей к приёмке, а стоимость списаний и уценки входит в "
            "целевую функцию наравне с хранением.\n\n"
            "BPMN: IO-020 — Расчет рекомендаций по уровням запасов (IO)"
        ),
        "analiz-poluchennyh-znacheniy": (
            "Сценарное моделирование и интерпретация полученных значений. Каждый сценарий "
            "— это воспроизводимый прогон с изменённым входом и вопросом, на который он "
            "отвечает.\n\n"
            "База (as is) — текущие политики и параметры, точка отсчёта для сравнения. "
            "Пересчёт по фильтрам — тот же расчёт на подмножестве: склад, канал, "
            "категория, сегмент; показывает, где эффект концентрируется и стоит ли катить "
            "на всю сеть. Сервис 98% по федеральным сетям — поднять целевой уровень там, "
            "где он контрактный, оставив остальным текущий: сколько запаса стоит "
            "обязательство и штрафы за его срыв. Учащённое пополнение охлаждённой "
            "продукции — частота с недельной на две в неделю: меньше циклического запаса "
            "и списаний против роста логистики. Сокращение лидтайма и его разброса — "
            "сколько страхового запаса высвобождает стабильность поставки, а не её "
            "скорость. Ограничение по мощностям холода — целевые уровни при фактической "
            "ёмкости по температурным режимам: что делать, если оптимум физически не "
            "помещается. Бюджет запаса −15% — фиксируем оборотный капитал и смотрим, чем "
            "платит сервис. Сезонный пик май–сентябрь — заблаговременный буфер по "
            "профильным отрубам под рост спроса.\n\n"
            "Перечень — МЕНЮ, а не план прогона: за цикл считаются три сценария, "
            "выбранные под задачу, и сравниваются с базой. Числовые пороги (98%, −15%) — "
            "предмет решения владельца, а не свойство модуля. Варианты будут уточнены по "
            "ходу проекта (сноска слайда).\n\n"
            "BPMN: IO-030 — Расчет альтернативного сценария с новыми входными данными"
        ),
        "analiz-preduprezhdeniy-rascheta": (
            "Разбор предупреждений, которые формирует расчёт: покрытие выше остаточного "
            "срока годности, рекомендация ниже минимальной партии, целевой запас не "
            "помещается в мощности хранения, отклонение от целевого уровня сервиса.\n\n"
            "Предупреждение — не ошибка расчёта, а позиция, где ограничение и "
            "рекомендация разошлись. Такие позиции уходят на ручную корректировку, а не "
            "молча остаются в выгрузке."
        ),
        "ocenka-effektov": (
...
STAGE_GROUPS: dict[str, dict[str, tuple[tuple[str, tuple[str, ...]], ...]]] = {
    ...
    "meio": {
        ...
        "stage-3-raschet-i-analiz": (
            ...
            ("Анализ", ("analiz-poluchennyh-znacheniy", "analiz-preduprezhdeniy-rascheta",)),
        ),
    },
}
