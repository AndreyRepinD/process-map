...
describe('карта MEIO: содержание', () => {
  ...
  it('база неопределённости — параметр настройки: расчёт в модуле ИЛИ ошибка из DP', () => {
    ...
    //
    // После перестройки развилка переехала со сборного шага настройки на
    // «Расчёт волатильности спроса» — именно он считает разброс и именно он
    // принимает готовую ошибку вместо собственного расчёта.
    ...
    expect(volatility?.description).toContain('ПАРАМЕТР НАСТРОЙКИ');
    expect(volatility?.description).toContain('собственной истории');
